...
if ( !empty($body) && $page_file != $main ) {
    ...
    if ( !empty($email) ){
        $author = '<a href="mailto:'.$email.'" target="_blank" rel="noopener noreferrer">'.$author.'</a>';
    }

    // If featured image is not missing and it is not set to 'none', insert it
    if ( !empty($image) && mb_strtolower($image) != "none" ){
        echo '<img src="/img/'.$image.'" width="640" height="360" style="margin: 1px auto 1px; display: block;">';
        echo '<hr width="75%">';
    }
